function StatBox({ value, label }) {
  return (
    <div className="flex flex-col items-center leading-none">
      ...
      <span className="text-red-600 text-[10px] sm:text-xs font-bold uppercase tracking-wide mt-1">{label}</span>
    </div>
  );
}
...
export default function CountdownDisplay({ data, stacked }) {
  ...
  return (
    <div className={`flex items-center gap-2 ${stacked ? "flex-col" : "flex-row"}`}>
      <span className={`text-red-600 font-bold text-sm sm:text-base leading-tight ${stacked ? "text-center" : ""}`}>
        {data.title}
      </span>
