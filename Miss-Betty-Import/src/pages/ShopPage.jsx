...
function mapProduct(p) {
  ...
  return {
    ...
    misc_amount: Number(p.misc_amount ?? 0), // admin-only; never rendered on this customer-facing page
    discount_price,
...
function ProductDetailModal({ product, onClose, buyNow = false }) {
  ...
  function addVariantToPending() {
    ...
    setPendingVariants(prev => {
      ...
      return [...prev, {
        ...
        rmbPrice: curRmbPrice, miscAmount: curMiscAmount,
  ...
  function commitToCart(andThen) {
    addMultipleToCart(
      pendingVariants.map(v => ({
        ...
        rmbPrice: v.rmbPrice, miscAmount: v.miscAmount,
  ...
  function handleSimpleAdd() {
    ...
    addToCart(product, curQty, null, null, curPrice, product.cost_price, product.profit, simpleOriginal, product.rmb_price, product.misc_amount);
    setAdded(true);
    ...
    navigate("/checkout", {
      state: {
        buyNow: {
          ...
          rmbPrice: product.rmb_price, miscAmount: product.misc_amount,
          originalPrice: curHasDiscount ? curRegularPrice : null,
        },
      },
    });
        ...
        <div className="px-3 py-2.5">
          ...
          {hasVariants ? (
            ...
            <div className="flex items-center gap-3 pb-2">
              ...
              {buyNow ? (
                <button onClick={handleSimpleBuyNow} disabled={blockedByPreorder} className={`flex-1 font-medium py-1.5 rounded-xl text-xs transition-opacity ${blockedByPreorder ? "bg-gray-200 text-gray-500 cursor-not-allowed" : "bg-[#F2AA25] text-white hover:opacity-90"}`}>
                  {blockedByPreorder ? "Pre-orders Closed" : "Proceed to Checkout"}
                </button>
              ) : (
                <button onClick={handleSimpleAdd} className={`flex-1 font-medium py-1.5 rounded-xl text-xs transition-colors ${added ? "bg-green-500 text-white" : "bg-[#F2AA25] text-white hover:opacity-90"}`}>
...
function ProductCard({ product, onSelect, onViewImage, onBuyNow, ordersClosed }) {
  ...
  function handleAdd(e) {
    ...
    const miscP     = sizeEntry?.misc_amount ?? product.misc_amount ?? 0;
    ...
    addToCart(product, 1, firstSize, firstColour, price, costP, prof, origPrice, rmbP, miscP);
    setAdded(true);
...
export default function ShopPage() {
  ...
  const [search, setSearch] = usePersistedState("mbimport_form_shop_filters_search", "");
  const [activeCategory, setActiveCategory] = usePersistedState("mbimport_form_shop_filters_category", "All");
  const [statusFilter, setStatusFilter] = usePersistedState("mbimport_form_shop_filters_status", "All");
