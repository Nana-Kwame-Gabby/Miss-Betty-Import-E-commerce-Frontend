  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="bg-white rounded-2xl shadow-sm p-8 text-center mb-6">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-[#1e2d3d] mb-2">Order Placed! 🎉</h1>
        <p className="text-gray-500 mb-4">
          Thank you, <strong>{form.fullName}</strong>! Your order has been received and is being processed.
        </p>
        <div className="inline-block bg-gray-50 border border-gray-100 rounded-xl px-5 py-3">
          <p className="text-xs text-gray-400 font-medium">Order ID</p>
          <p className="text-[#F2AA25] font-bold text-lg">{orderId}</p>
        </div>
      </div>

      {/* Order details */}
      <div className="bg-white rounded-2xl shadow-sm p-6 mb-6">
        <h2 className="font-bold text-[#1e2d3d] mb-4">Order Details</h2>
        <div className="flex flex-col gap-3 mb-4">
          {items.map(item => (
            <div key={item.cartKey} className="flex items-center gap-3">
              <img src={item.product_image_url} alt={item.product_name} className="w-12 h-14 object-cover rounded-xl" />
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 mb-8">
        <h2 className="font-bold text-[#1e2d3d] mb-3">Delivery To</h2>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <Link
          to="/shop"
          className="flex-1 text-center bg-[#F2AA25] text-white font-bold py-4 rounded-2xl hover:opacity-90 transition-opacity"
        >
          Continue Shopping
        </Link>
        <Link
          to="/my-orders"
          className="flex-1 text-center border-2 border-[#1e2d3d] text-[#1e2d3d] font-bold py-4 rounded-2xl hover:bg-[#1e2d3d] hover:text-white transition-colors"
        >
          View My Orders
        </Link>
      </div>
