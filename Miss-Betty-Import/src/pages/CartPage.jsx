...
export default function CartPage() {
  ...
  if (cartItems.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-24 text-center">
        ...
        <h2 className="text-2xl font-bold text-[#1e2d3d] mb-2">Your cart is empty</h2>
        <p className="text-gray-400 mb-8">Looks like you haven't added anything yet.</p>
        <Link
          to="/shop"
          className="inline-block bg-[#F2AA25] text-white font-bold px-8 py-3 rounded-2xl hover:opacity-90 transition-opacity"
        >
          Start Shopping
        </Link>
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-2xl sm:text-3xl font-bold text-[#1e2d3d] mb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          {cartItems.map(item => (
            <div key={item.cartKey} className="bg-white rounded-2xl shadow-sm p-4 flex gap-4 items-start">
              <img
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-24">
            ...
            <button
              ...
              className="w-full bg-[#F2AA25] text-white font-bold py-4 rounded-2xl hover:opacity-90 transition-opacity"
            >
              Proceed to Checkout
            </button>
            <Link
